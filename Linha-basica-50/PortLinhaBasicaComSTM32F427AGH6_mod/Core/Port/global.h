...
EXT BYTE BufferMovimentacao[1024*2];
...
EXT unsigned short const mask[17];
...
extern const unsigned short  mask[17];

extern unsigned long const baudrates[8];
EXT BYTE BufferMovimentacao[1024*2];//  __at (0x40001000) ;/* Buffer Temporario para movimentacao de dados na Flash*/
...
extern unsigned int CounterMASK[16];
extern float const polinomioMIa[4];
extern float const polinomioMIb[4];
extern float const polinomioMIc[4];

extern char TAB_SIL_CARGA[8];
...
/////////////////////////////relacionados ao novo chip da ST///////////////////qq

#define ARM_MATH_CM4
#include <arm_math.h>

#define NUM_CHANNELS		6
#define FFT_LEN				1024  //testar! porque 1024 parece que não é permitido!!!

#define FFT_HALF_LEN		(FFT_LEN>>1)

//Para STM32F746 e para STM32F429: The timer 2 is on APB1 bus, the maximum of clock frequency of APB1 timer clock is half of SYSCLK.
//Para STM32F429 o timer 2, 5, 3 e timer 4 estão no ABP1 bus.

//#define FREQ_APB			216000000 108000000
#define FREQ_APB			90000000

#define FFTs_PER_SEC		5 			  //número de FFTs por segundo por canal. ex. 10
#define FREQ_AMOSTRAGEM_DAC (FFT_LEN*FFTs_PER_SEC)  //10240.156040472997683774228940631//
#define FREQ_AMOSTRAGEM		(FFT_LEN*FFTs_PER_SEC)  //ex fs=2048*5 =10240hz, ou seja 2048 pontos a cada 1/5 de segundo (200ms)!, há que ver se o processador consegue processar tudo isso en 200ms.
    									  	  	  	//ex fs=1024*10=10240hz, ou seja 1024 pontos a cada 1/10 de segundo (100ms)!, há que ver se o processador consegue processar tudo isso en 100ms.
													//ex fs=1024*6=6144hz, ou seja 1024 pontos a cada 1/6 de segundo (166.66ms)!, há que ver se o processador consegue processar tudo isso en 166ms.
                                                     // ex fs=1024*5=5120hz, ou seja 1024 pontos a cada 1/5 de segundo (200ms)!, há que ver se o processador consegue processar tudo isso en 200ms.

#define CONTAGEM_1SEG	(FFTs_PER_SEC*NUM_CHANNELS)

#define ADC1_BUF_LEN 	((FFT_LEN*NUM_CHANNELS*2) ) //(half+complete), 6canais
#define ADC2_BUF_LEN 	((FFT_LEN*NUM_CHANNELS*2)>>1) //(half+complete), 3canais

#define DAC_BUF_LEN 	(FFT_LEN)



#define FS                              FREQ_AMOSTRAGEM
#define NPONTOS_ESPECTRAIS_DFT          FFT_HALF_LEN     //número de componentens da DFT (não espelhadas)

#define f1 60
#define NPONTOS_ESPECTRAIS_INTERP       50     //numero máximo de componentes interpoladas
#define JANELAMENTO_RETANGULAR
//#define JANELAMENTO_HANNING
...
//instanciando a estrutura de FFT
EXT   arm_rfft_fast_instance_f32 MyFFT;

EXT   float PhaseFund;
EXT   float FreqFund ;
EXT   int ChannFund;

EXT   volatile  int32_t timer_val;
EXT   volatile  uint32_t timer_flag;

EXT    volatile int flag_processando_half1 ;
EXT   volatile  int flag_processando_half2 ;
EXT    volatile int flag_processando_full1 ;
EXT    volatile int flag_processando_full2 ;

EXT    volatile int flag_processado_half1 ;
EXT   volatile  int flag_processado_half2 ;
EXT    volatile int flag_processado_full1 ;
EXT    volatile int flag_processado_full2 ;


EXT   volatile  int flag_hab_processa_half1 ;
EXT   volatile  int flag_hab_processa_half2 ;
EXT    volatile int flag_hab_processa_full1 ;
EXT    volatile int flag_hab_processa_full2 ;

EXT   volatile int flag_acavalou ;
EXT   volatile int flag_flushADCbuffers;

EXT    volatile uint16_t adc1_data_buffer[ADC1_BUF_LEN];
//EXT   volatile  uint16_t adc2_data_buffer[ADC1_BUF_LEN];
EXT  volatile   uint16_t dac1_data_buffer[DAC_BUF_LEN];
EXT  volatile   uint16_t dac2_data_buffer[DAC_BUF_LEN];

EXT   volatile  float32_t pSinal[FFT_LEN];
EXT   volatile  float32_t ch0_buffer[FFT_LEN];
EXT    volatile float32_t ch1_buffer[FFT_LEN];
EXT    volatile float32_t ch2_buffer[FFT_LEN];
EXT    volatile float32_t ch3_buffer[FFT_LEN];
EXT   volatile  float32_t ch4_buffer[FFT_LEN];
EXT   volatile  float32_t ch5_buffer[FFT_LEN];


EXT  bin_phasor_interp_t pEspectroAcumulado[NUM_CHANNELS][NPONTOS_ESPECTRAIS_INTERP];
EXT   bin_phasor_interp_t pEspectroDisplay[NUM_CHANNELS][NPONTOS_ESPECTRAIS_INTERP];

EXT   int cont_espectros;


EXT long int next_calib_addr;
EXT long int curr_calib_addr;


EXT   uint16_t globalVarSWV1;

EXT bin_phasor_interp_t pOutTemporario[NPONTOS_ESPECTRAIS_INTERP];
#ifdef JANELAMENTO_HANNING
EXT float32_t   janela[FFT_LEN];
