...
DWORD ADCInit( DWORD ADC_Clk )
{
	...
	AD0CR = ( 0x01 << 0 ) | 	/* SEL=1,select channel 0~7 on ADC0 */
		( ( Fpclk_72Mhz / ADC_Clk - 1 ) << 8 ) |  /* CLKDIV = Fpclk / 1000000 - 1 */ 
...
void ADCStartV( BYTE channelNum0)
{
    #ifdef LEGADO
	AD0CR &= 0xF8FFFFFF;	   //stop ADC now
	AD0CR &= 0xFFFFFF00;
	AD0CR |= (1 << 24) | (1 << channelNum0);
	#endif
}
...
short ADCReadV(void)
{
	...
	// while ( 1 )			/* wait until end of A/D convert */
	// {
	regVal= AD0GDR; // read result of A/D conversion 	
	//regVal = *(volatile unsigned long *)(AD0_BASE_ADDR 
	//			+ ADC_OFFSET + ADC_INDEX * channelNum0);
	...
	//}
	//	pisca2();	  
	return((short)( regVal >> 6 ) & 0x3FF); //tranca aqui
	//*pCH0=(short)( regVal >> 6 ) & 0x3FF;
	//	pisca2();
#endif
}
...
DWORD ADC0Read( BYTE channelNum )
{
	...
	ADC_Data = ( regVal >> 6 ) & 0x3FF;
	return ( (short) ADC_Data );	/* return A/D conversion value */
#else
	return ( channelNum );	/* if it's interrupt driven, the ADC reading is 
							   done inside the handler. so, return channel number */
#endif
